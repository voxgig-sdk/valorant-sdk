...
(0, node_test_1.describe)('WeaponDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-weapon', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-weapon', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-weapon', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-weapon', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.VALORANT_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.ValorantSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['VALORANT_TEST_WEAPON_ENTID'];
    ...
    const client = new __1.ValorantSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=WeaponDirect.test.js.map
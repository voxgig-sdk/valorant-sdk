...
//     (\`new ValorantSDK(...)\` / \`ValorantSDK.test(...)\` become
//     \`ValorantSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"agent":{"test01":{"id":"test01"}},"competitive":{"test01":{"id":"test01"}},"cosmetic":{"test01":{"id":"test01"}},"game_mode":{"test01":{"id":"test01"}},"map":{"test01":{"id":"test01"}},"weapon":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ValorantSDK(...)\` and any
// \`ValorantSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ValorantSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ValorantSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

...
class TestMapEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Map(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Map(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _map_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "map." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set VALORANT_TEST_MAP_ENTID JSON to run live")
        client = setup["client"]
...
def _map_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/map/MapTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("VALORANT_TEST_MAP_ENTID"))

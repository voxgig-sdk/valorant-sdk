# Valorant SDK test runner

from __future__ import annotations
import os
import json

from valorant_sdk.utility.voxgig_struct import voxgig_struct as vs


class ValorantTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ValorantTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ValorantTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ValorantTestRunner.getenv("VALORANT_TEST_LIVE")
        override = ValorantTestRunner.getenv("VALORANT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ValorantTestRunner.getenv(key)
        ...
        explain = ValorantTestRunner.getenv("VALORANT_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["VALORANT_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ValorantTestRunner._test_control is not None:
            return ValorantTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ValorantTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ValorantTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ValorantTestRunner.load_test_control()
...
def load_env_local():
    ValorantTestRunner.load_env_local()


def env_override(m):
    return ValorantTestRunner.env_override(m)


def entity_data(v):
    return ValorantTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ValorantTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ValorantTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ValorantTestRunner.load_test_control()


def live_client_options():
    return ValorantTestRunner.live_client_options()


def live_delay_ms():
    return ValorantTestRunner.live_delay_ms()

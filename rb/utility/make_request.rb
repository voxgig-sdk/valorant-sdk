...
module ValorantUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = ValorantResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ValorantResponse.new(fetched)
    else

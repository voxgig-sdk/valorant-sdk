...
class CompetitiveDirectTest < Minitest::Test
  def test_direct_list_competitive
    setup = competitive_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-competitive", setup[:live] ? "live" : "unit")
    if _should_skip
...
def competitive_direct_setup(mockres)

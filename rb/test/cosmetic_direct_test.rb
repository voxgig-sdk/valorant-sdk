...
class CosmeticDirectTest < Minitest::Test
  def test_direct_list_cosmetic
    setup = cosmetic_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-cosmetic", setup[:live] ? "live" : "unit")
    if _should_skip
...
def cosmetic_direct_setup(mockres)

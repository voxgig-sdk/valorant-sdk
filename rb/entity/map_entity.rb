...
class MapEntity
  ...
  def make
    opts = @_entopts.dup
    MapEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ValorantHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Map, Hash] the current Map data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ValorantHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Map fields)
  def match_get
  ...
  # Load a single Map.
  #
  # @param reqmatch [MapLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Map.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Map, Hash] the loaded Map; raises ValorantError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ValorantHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [MapListMatch, Hash, nil] match filter (any subset of
  #   Map fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Map>, Array] the matching Map items; raises ValorantError on failure

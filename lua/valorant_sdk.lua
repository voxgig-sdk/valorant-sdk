...
-- by passing it.
function ValorantSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ValorantSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ValorantSDK: graphql: " .. msg
    res.graphql = errors
  end

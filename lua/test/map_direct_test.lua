...
describe("MapDirect", function()
  it("should direct-list-map", function()
    local setup = map_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-map", setup.live and "live" or "unit")
  ...
  it("should direct-load-map", function()
    local setup = map_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-map", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function map_direct_setup(mockres)

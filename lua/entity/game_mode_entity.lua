...
GameModeEntity.__index = GameModeEntity


function GameModeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GameModeEntity)
  self._name = "game_mode"
  self._client = client
...
function GameModeEntity:get_name()
  return self._name
end


function GameModeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GameModeEntity.new(self._client, opts)
end
...
function GameModeEntity:mark_deleted()
  self._deleted = true
end


function GameModeEntity:deleted()
  return true == self._deleted
end


function GameModeEntity:data_set(args)
  if args ~= nil then
...
function GameModeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GameModeEntity:match_set(args)
  if args ~= nil then
...
function GameModeEntity:match_get()
...
function GameModeEntity:stream(action, args, callopts)
...
function GameModeEntity:_run_op(ctx, post_done)
  local utility = self._utility

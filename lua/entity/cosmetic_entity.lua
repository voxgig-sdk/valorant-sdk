...
local CosmeticEntity = {}
CosmeticEntity.__index = CosmeticEntity


function CosmeticEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CosmeticEntity)
  self._name = "cosmetic"
  self._client = client
...
function CosmeticEntity:get_name()
  return self._name
end


function CosmeticEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CosmeticEntity.new(self._client, opts)
end
...
function CosmeticEntity:mark_deleted()
  self._deleted = true
end


function CosmeticEntity:deleted()
  return true == self._deleted
end


function CosmeticEntity:data_set(args)
  if args ~= nil then
...
function CosmeticEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CosmeticEntity:match_set(args)
  if args ~= nil then
...
function CosmeticEntity:match_get()
...
function CosmeticEntity:stream(action, args, callopts)
...
function CosmeticEntity:_run_op(ctx, post_done)
  local utility = self._utility

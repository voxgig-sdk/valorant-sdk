...
local CompetitiveEntity = {}
CompetitiveEntity.__index = CompetitiveEntity


function CompetitiveEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompetitiveEntity)
  self._name = "competitive"
  self._client = client
...
function CompetitiveEntity:get_name()
  return self._name
end


function CompetitiveEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompetitiveEntity.new(self._client, opts)
end
...
function CompetitiveEntity:mark_deleted()
  self._deleted = true
end


function CompetitiveEntity:deleted()
  return true == self._deleted
end


function CompetitiveEntity:data_set(args)
  if args ~= nil then
...
function CompetitiveEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompetitiveEntity:match_set(args)
  if args ~= nil then
...
function CompetitiveEntity:match_get()
...
function CompetitiveEntity:stream(action, args, callopts)
...
function CompetitiveEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
local MapEntity = {}
MapEntity.__index = MapEntity


function MapEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MapEntity)
  self._name = "map"
  self._client = client
...
function MapEntity:get_name()
  return self._name
end


function MapEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MapEntity.new(self._client, opts)
end
...
function MapEntity:mark_deleted()
  self._deleted = true
end


function MapEntity:deleted()
  return true == self._deleted
end


function MapEntity:data_set(args)
  if args ~= nil then
...
function MapEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MapEntity:match_set(args)
  if args ~= nil then
...
function MapEntity:stream(action, args, callopts)
...
---@param reqmatch MapLoadMatch
---@param ctrl? table
---@return Map
---@return string? err
function MapEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function MapEntity:_run_op(ctx, post_done)
  local utility = self._utility

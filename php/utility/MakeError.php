...
class ValorantMakeError
{
    public static function call(?ValorantContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ValorantContext([], null);
        }
        $op = $ctx->op ?? new ValorantOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ValorantResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ValorantError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ValorantError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof ValorantError) {

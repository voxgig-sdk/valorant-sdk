...
class GameModeEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ValorantSDK::test(null, null);
        $ent = $testsdk->GameMode(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->GameMode(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->GameMode(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = game_mode_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "game_mode." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set VALORANT_TEST_GAME_MODE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $game_mode_ref01_data = null;
        if (count($game_mode_ref01_data_raw) > 0) {
...
function game_mode_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/game_mode/GameModeTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["game_mode01", "game_mode02", "game_mode03"] as $k) {
    ...
    $entid_env_raw = getenv("VALORANT_TEST_GAME_MODE_ENTID");

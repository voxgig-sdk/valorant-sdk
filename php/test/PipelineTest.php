...
{
    private static function utility(): ValorantUtility
    {
        return new ValorantUtility();
    }
    ...
    private static function ctx(array $over = []): ValorantContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ValorantContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ValorantOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ValorantError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ValorantOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ValorantMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ValorantOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ValorantMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ValorantOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ValorantMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ValorantSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ValorantMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ValorantResponse([]),
            'result' => new ValorantResult([]),
        ]);
        [, $err] = ValorantMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ValorantSpec([]),
            'response' => null,
            'result' => new ValorantResult([]),
        ]);
        [, $err] = ValorantMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ValorantSpec(['step' => 's']),
            'response' => new ValorantResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ValorantResult([]),
        ]);
        [, $err] = ValorantMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ValorantSpec(['step' => 's']),
            'response' => new ValorantResponse(self::resp(200, ['v' => 1])),
            'result' => new ValorantResult([]),
        ]);
        [, $err] = ValorantMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ValorantSpec(['step' => 's']),
            'response' => new ValorantResponse(self::resp(200, ['v' => 2])),
            'result' => new ValorantResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ValorantMakeResponse::call($ctx);
    ...
    {
        $preset = new ValorantResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ValorantSpec([]),
            'response' => new ValorantResponse([]),
            'result' => new ValorantResult([]),
        ...
        [$got, $err] = ValorantMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ValorantResult([])]);
        [, $err] = ValorantMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ValorantSpec([]), 'result' => null]);
        [, $err] = ValorantMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ValorantSpec(['step' => 's']),
            'result' => new ValorantResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ValorantOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ValorantMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ValorantSpec(['step' => 's']),
            'result' => new ValorantResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ValorantOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ValorantMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ValorantResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ValorantSpec([]),
            'result' => new ValorantResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ValorantMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ValorantMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ValorantError('boom', 'boom');
        $utility->fetcher = function (ValorantContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ValorantSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ValorantMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ValorantContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ValorantMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ValorantResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ValorantContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ValorantSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ValorantMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ValorantResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ValorantSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ValorantMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ValorantMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ValorantMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ValorantResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ValorantDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ValorantResult(['ok' => false])]);
        $this->expectException(ValorantError::class);
        ValorantDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ValorantResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ValorantMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ValorantMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ValorantBaseFeature();
        $b = new ValorantBaseFeature();
        ValorantFeatureAdd::call($ctx, $a);
        ValorantFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ValorantBaseFeature
    {
        $f = new ValorantBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ValorantFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ValorantContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ValorantSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ValorantPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ValorantResponse(['headers' => null]),
            'result' => new ValorantResult([]),
        ]);
        ValorantResultHeaders::call($ctx);

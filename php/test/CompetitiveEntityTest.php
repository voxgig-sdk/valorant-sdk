...
class CompetitiveEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ValorantSDK::test(null, null);
        $ent = $testsdk->Competitive(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Competitive(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Competitive(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = competitive_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "competitive." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set VALORANT_TEST_COMPETITIVE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $competitive_ref01_data = null;
        if (count($competitive_ref01_data_raw) > 0) {
...
function competitive_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/competitive/CompetitiveTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["competitive01", "competitive02", "competitive03"] as $k) {
    ...
    $entid_env_raw = getenv("VALORANT_TEST_COMPETITIVE_ENTID");

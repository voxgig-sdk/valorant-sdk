...
class CosmeticEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CosmeticEntity($this->_client, $opts);
    }

    /**
     * @param Cosmetic|array $args Cosmetic data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ValorantHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Cosmetic|array The current Cosmetic data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Cosmetic fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ValorantHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Cosmetic fields).
     */
    ...
    /**
     * List Cosmetic items matching the given filter.
     *
     * @param CosmeticListMatch|array|null $reqmatch Match filter (any subset
     *   of Cosmetic fields) as an assoc-array; CosmeticListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Cosmetic[]|array A list of Cosmetic items as assoc-arrays at

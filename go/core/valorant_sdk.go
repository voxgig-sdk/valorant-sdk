...
func (sdk *ValorantSDK) OptionsMap() map[string]any {
...
func (sdk *ValorantSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *ValorantSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *ValorantSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *ValorantSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *ValorantSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *ValorantSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("ValorantSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *ValorantSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *ValorantSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("ValorantSDK: graphql: %s", msg)
		res["graphql"] = errors
	}

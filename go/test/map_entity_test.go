...
func TestMapEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Map(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Map(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		mapRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.map")))
		...
		mapRef01DataDt0LoadResult := core.ToMapAny(entityData(mapRef01DataDt0Loaded))
		if mapRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if mapRef01DataDt0LoadResult["id"] != mapRef01Data["id"] {

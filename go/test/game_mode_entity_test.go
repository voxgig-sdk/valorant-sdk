...
func TestGameModeEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GameMode(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GameMode(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		gameModeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.game_mode")))

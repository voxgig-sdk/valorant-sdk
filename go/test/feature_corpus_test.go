...
//
// NewValorantSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.ValorantSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewValorantSDK(opts)
}
...
func fcCandidates(client *sdk.ValorantSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.ValorantSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.ValorantSDK, name string) any {

...
import (
	...
	omni "github.com/voxgig-sdk/valorant-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ValorantError); is {
				out := map[string]any{
					"name":    "ValorantError",
...
func omniCtx(arg any, client *sdk.ValorantSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ValorantSDK); is {
			client = live

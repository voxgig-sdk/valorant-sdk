...
func TestMapDirect(t *testing.T) {
	t.Run("direct-list-map", func(t *testing.T) {
		setup := mapDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-map", _mode); _shouldSkip {
			if _reason == "" {
...
func mapDirectSetup(mockres any) *mapDirectSetupResult {
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["VALORANT_TEST_MAP_ENTID"]; ok {

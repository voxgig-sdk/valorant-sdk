...
MCP server exposing the Valorant SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `valorant_list` | `entity`, optional `query` map | First page of records as JSON |
| `valorant_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `valorant_list` and
`valorant_load` tools become available in new sessions.

...
type Args struct {
	Entity string         `json:"entity" jsonschema:"agent | competitive | cosmetic | game_mode | map | weapon"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ValorantSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "valorant_list",
		Description: "List records from Valorant. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "valorant_load",
		Description: "Load a single record from Valorant. " +
...
func runOp(client *sdk.ValorantSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ValorantSDK, name string) (sdk.ValorantEntity, error) {
	switch strings.ToLower(name) {
	case "agent":
		return client.Agent(nil), nil
	case "competitive":
		return client.Competitive(nil), nil
	case "cosmetic":
		return client.Cosmetic(nil), nil
	case "game_mode":
		return client.GameMode(nil), nil
	case "map":

...
// Valorant SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class ValorantMakePoint
{
    public static function call(ValorantContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof ValorantError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = ValorantHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = ValorantHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));

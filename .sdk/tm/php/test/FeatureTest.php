...
class FtCtrl extends ValorantControl
{
...
class FtClient
{
    ...
    private ValorantUtility $utility;

    public function __construct(ValorantUtility $utility, array $options)
    {
    ...
    public function get_utility(): ValorantUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ValorantContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ValorantContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ValorantContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ValorantConfig::make_feature($name);
    ...
    private static function buildUrl(ValorantSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ValorantError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ValorantResponse($fetched);
        }
    ...
    private function fail(ValorantContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ValorantContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

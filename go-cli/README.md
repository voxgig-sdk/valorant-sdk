...
boru-driven command-line client **and** interactive REPL for the Valorant
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/valorant-cli)
make build
...
./valorant-cli list agent
./valorant-cli load 1 agent            # {id:1} shorthand
./valorant-cli load '{id:1}' agent       # explicit match map
./valorant-cli list competitive

# 5. Override the API base URL for a single call
VALORANT_BASE=https://api.example.com ./valorant-cli list agent
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/valorant-cli
   ```
...
```sh
./valorant-cli load 1 agent          # scalar shorthand for {id:1}
./valorant-cli load '{id:1}' agent     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `valorant>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/valorant-cli`. |
...
(`./valorant-cli <expr>`) and interactively (the REPL), and expressions compose the
